...
function accumulate(){
    ...
    switch(chtype+1){
        ...
        case 4:
            ...
            Type.info="<div id = 'critical'>我</div>が道を行くマイペースで、些細なはとらわれない寛容な心を持っています<br>集団よりも一人行動が好きで、猫のようにふらっとどこかに出かけていきます。基本的にインドアで冬はこたつから離れらません。もこもこした服、ゆるっとした服がよく似合うでしょう。寒い冬に出かけないといけない時は、充電できるカイロを持って外出中もぬくぬくしましょう♨";   // 基本情報
            Type.item="充電式カイロ";  // アイテム
            Type.img="result4.png";   // キャラ画像
            Type.beans="モバイルバッテリー×充電式カイロ"; //豆知識
            Type.beansSub="さまざまな充電式カイロが売られていますが、冬しか使えないのは少し物足りないという方にはモバイルバッテリーの機能も兼ね備えている充電式カイロがおすすめです!<br>'mottole MTL-E029'というカイロはスイッチを入れると5秒で暖まり、5～8時間は連続使用できます。1年中活用できるだけでなく、地震による停電などの緊急時でも使用可能です。他のUSB機器への充電もできるため、普段はモバイルバッテリーとして持ち運び、寒い冬にはカイロとして活躍してくれる優れものです!";  // 基本情報
            break;
        ...
        case 5:
            ...
            Type.beansSub="「割り箸は間伐材を利用しているから環境に良い」と聞いたことはありませんか？実際環境負荷が少ないのは、割り箸とマイ箸、どっちなのでしょうか？<br>これは割り箸の生産国が日本か外国かで異なります。仮想店舗で1年間使用した場合を比較すると、マイ箸は年間[67kg]のCO2、国産の割り箸は[62kg]のCO2を材料調達から廃棄までで排出すると言われています。一方、中国産の割り箸だと年間[15,167kg]ものCO2を排出します。これは材料調達にかかるCO2量が大半を占めるからです。<br>マイ箸よりも国産の割り箸を使っていけば問題ありません。しかし外食産業で用いられる割り箸の約98%が外国産で、国産のものはわずか2%程です。毎回、国産の割り箸を手にすることは出来ないでしょう。本当に環境負荷が小さい箸はどっちでしょうか？<div id = 'small'>参考資料「割り箸と洗い箸のライフサイクルCO2比較分析」朝倉・小川 , 2014 , 第9回日本LCA学会研究発表会講演要旨集</div>";
            break;

        case 6:
            Type.color="#407936"; //テーマカラ-
            Type.info="<div id = 'critical'>効</div>率的、論理的な知性派です。感情よりも論理性を重視しますが、決断する時は直感的になりがち。ユニクロなどシンプルな服装を着がちですが、柄シャツを選ぶと一気にオシャレの幅が広がるでしょう。パソコンなどの電子機器を使う場面が多いので、複数の機器を充電できて、かつ節電にもなる節電タップがオススメです。お洒落で格好良い理系大学生を目指しませんか？";   // 基本情報
            Type.item="節電タップ";  // アイテム
            Type.img="result6.png";   // キャラ画像
            Type.beans="待機電力のはなし"; //豆知識分
            Type.beansSub="待機電力とは電子機器を使っていなくてもコンセントに繋いでいるだけで消費される電力のことです。待機電力の消費電力量は一世帯あたり年間で228kWと推定され、電気代に換算すると年間約6,156円も無駄にしています。<br>待機電力を無くすには電子機器をコンセントから抜く必要がありますが、毎回外すのは面倒ですよね。そこで役に立つのが節電タップ!節電タップは各コンセントの挿し口にON･OFFのスイッチが付いており、使わないときはスイッチを押すだけなので簡単に待機電力分を節約することが出来ます。節電タップを使用するだけで、待機電力を約50%も削減できます。節約･節電の為に、通常のたこ足配線はやめて節電タップにしませんか？";
            break;
    ...
    let beans =  '<div class = "beans"> <div id="top"><br>【'+Type.beans+'】<br></div><div id="normal">'+Type.beansSub+'<br></div></div>';
