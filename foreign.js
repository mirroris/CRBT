...
function accumulate(){
    ...
    switch(chtype+1){
        case 1:
            ...
            Type.beansSub="写真"<br>
            'デンマークの環境食品省環境保護機関は「ライフサイクルアセスメント(LCA)」という環境負荷を総合評価する手法を用いて、1枚のレジ袋を作る上で発生する環境負荷の試算を発表しました。<br>その結果によると、アパレル店でよく用いられるLDPE素材のレジ袋を1回再利用(一度ゴミ袋として利用)することは、<br>●エコバッグ(ポリステルバッグ)を35回再使用する<br>●紙袋を43回再利用する<br>●オーガニックコットンバッグ20000回再使用する<br>と同程度の環境負荷になります。<br>エコバックは数回使うだけでは意味が無く、何回も繰り返し使うことでどんどんエコになっていきます。<br>使う頻度や耐久性、金額なども踏まえて、自分のマイバックを探してみましょう！<br>';
